...
import { appointmententity } from '../appointment/appointmententity';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';
import { doctorentity } from '../appointment/doctorentity';
...
getDoctorByDepartment(department:string){
  return this.http.get<doctorentity[]>(`http://localhost:8000/admin/search/${department}`).pipe(map((res:any)=>{return res;}));
}
